...
1. Start voor native product-blocks, theme blocks en template placement eerst met `plan-theme-edit` op hetzelfde expliciete theme. Die planner geeft compact terug of je een single-file patch, multi-file edit of create-section flow nodig hebt. Voor `new_section` retourneert de planner nu ook compacte `themeContext`-guardrails uit een content-achtige referentiesection, zodat een LLM vooraf de bestaande page-width, spacing-settings en schaalconventies kan spiegelen in plaats van blind hero-achtige waardes te kiezen.
2. Zoek de gewenste code met `search-theme-files` om een betrouwbare `searchString` te verkrijgen. Gebruik na `plan-theme-edit` bij voorkeur exact de `nextReadKeys`, zodat je compact binnen section/snippet-bestanden zoekt in plaats van hele templates opnieuw te lezen.
3. Lees daarna met `get-theme-file` alleen het exacte doelbestand in dat je gaat aanpassen. Voor native product-blocks is dat meestal de bestaande section en eventuele snippet renderer; herlees `templates/*.json` alleen als placement expliciet gevraagd is.
4. Voer nieuwe losse section-creaties bij voorkeur uit via `create-theme-section`. Gebruik `draft-theme-artifact` in `mode="edit"` voor bestaande bestanden en in `mode="create"` voor advanced create-flows of multi-file writes. Gebruik bij voorkeur `patch` voor één gerichte wijziging of `patches` voor meerdere sequentiële wijzigingen in hetzelfde bestand om token-verbruik te beperken en truncation te voorkomen. De pipeline valideert section/block schema-ranges nu ook vooraf op bounds, step-alignment, kleine discrete ranges en de Hazify preflight-guard van maximaal 101 stappen. Bij een foutieve range-default geeft de tool nu ook exacte `suggestedReplacement.default` en geldige candidate-defaults terug zodat een retry minder hoeft te gokken.
5. `create-theme-section` leidt intern nogmaals compacte theme-context af voordat de daadwerkelijke guarded create-flow draait. Daardoor kan dezelfde request niet alleen schema/lint-fouten afvangen, maar ook hero-achtige oversizing van typography, spacing, gaps en min-heights blokkeren wanneer die duidelijk buiten de content-conventies van het doeltheme vallen.
6. Lokale create-validatie bundelt nu meerdere deterministische fouten in één response met machine-readable velden zoals `errorCode`, `errors[]`, `lintIssues[]`, `normalizedArgs`, `nextAction`, `retryMode`, `suggestedSchemaRewrites`, `preferSelectFor` en waar relevant `themeContext`. Operationele blockers zoals patch-anchor fouten, mixed create/edit zonder mode, read failures en preview write failures blijven fail-fast. Shopify richtext-default fouten worden explicieter teruggegeven via codes zoals `richtext_default_forbidden_tag`; te minimale premium standalone sections geven `standalone_section_too_minimal`. Als zowel lokale inspectie als lint stuk zijn, wordt dat nu in dezelfde preflight-response geretourneerd.
