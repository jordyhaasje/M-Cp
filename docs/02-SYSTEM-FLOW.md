...
## 4. Toolcatalogus
<!-- BEGIN: TOOLS_LIST -->
- **`add-tracking-to-order`**: Legacy alias of set-order-tracking. Prefer set-order-tracking for new tool calls; this alias is kept only for compatibility.
...
- **`read-theme-file`**: Legacy alias of get-theme-file. Prefer get-theme-file for new tool calls; this alias is kept only for compatibility with clients that guess read-* naming.
- **`read-theme-files`**: Legacy alias of get-theme-files. Prefer get-theme-files for new tool calls; this alias is kept only for compatibility with clients that guess read-* naming.
...
- **`update-order-tracking`**: Legacy alias of set-order-tracking. Prefer set-order-tracking for new tool calls; this alias is kept only for compatibility.
...
- Canonical agent flow: `get-themes` -> `plan-theme-edit` -> compacte read/preflight -> `draft-theme-artifact mode="create"` voor nieuwe sections -> optioneel aparte `mode="edit"` of `patch-theme-file` voor vervolgfixes.
...
1. Start voor native product-blocks, theme blocks en template placement eerst met `plan-theme-edit` op hetzelfde expliciete theme. Die planner geeft compact terug of je een single-file patch, multi-file edit of create-section flow nodig hebt.
...
4. Voer de edit uit via `draft-theme-artifact` in `mode="edit"` of `mode="create"`. Gebruik bij voorkeur `patch` voor één gerichte wijziging of `patches` voor meerdere sequentiële wijzigingen in hetzelfde bestand om token-verbruik te beperken en truncation te voorkomen. De pipeline valideert section/block schema-ranges nu ook vooraf op bounds, step-alignment, kleine discrete ranges en de Hazify preflight-guard van maximaal 101 stappen. Bij een foutieve range-default geeft de tool nu ook exacte `suggestedReplacement.default` en geldige candidate-defaults terug zodat een retry minder hoeft te gokken.
5. Lokale create-validatie bundelt nu meerdere deterministische fouten in één response met machine-readable velden zoals `errorCode`, `errors[]`, `lintIssues[]`, `normalizedArgs`, `nextAction`, `retryMode`, `suggestedSchemaRewrites` en `preferSelectFor`. Operationele blockers zoals patch-anchor fouten, mixed create/edit zonder mode, read failures en preview write failures blijven fail-fast. Shopify richtext-default fouten worden explicieter teruggegeven via codes zoals `richtext_default_forbidden_tag`; te minimale premium standalone sections geven `standalone_section_too_minimal`. Als zowel lokale inspectie als lint stuk zijn, wordt dat nu in dezelfde preflight-response geretourneerd.
6. Live of ander target toepassen gebeurt via `apply-theme-draft` met expliciete confirmation.
7. Verifieer writes via de `verify` output van `draft-theme-artifact` of aanvullend met `verify-theme-files` of `get-theme-file`.
8. Als een gebruiker placement op homepage/producttemplate expliciet vraagt, doe dat als aparte `mode="edit"` wijziging op het juiste `templates/*.json` bestand van hetzelfde gekozen theme.
