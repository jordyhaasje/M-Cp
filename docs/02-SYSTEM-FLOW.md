...
7. Lokale create-validatie bundelt nu meerdere deterministische fouten in één response met machine-readable velden zoals `errorCode`, `errors[]`, `lintIssues[]`, `normalizedArgs`, `nextAction`, `retryMode`, `suggestedSchemaRewrites`, `preferSelectFor` en waar relevant `themeContext` en `sectionBlueprint`. Operationele blockers zoals patch-anchor fouten, mixed create/edit zonder mode, read failures en preview write failures blijven fail-fast. Shopify richtext-default fouten worden explicieter teruggegeven via codes zoals `richtext_default_forbidden_tag`; te minimale premium standalone sections geven `standalone_section_too_minimal`. Parser-onveilige `${...{{ ... }}...}` combinaties, geneste `{{ ... }}` binnen dezelfde output-tag en niet-gescopede interactieve JS worden nu ook vóór preview-write gevangen. Als zowel lokale inspectie als lint stuk zijn, wordt dat nu in dezelfde preflight-response geretourneerd.
