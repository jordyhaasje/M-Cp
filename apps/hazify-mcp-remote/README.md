...
- `plan-theme-edit` geeft nu de onmiddellijke volgende stap terug via `nextTool` en `nextArgsTemplate`: native blocks en patch-first editflows gaan bij voorkeur eerst via compacte `search-theme-files` anchors, terwijl bredere rewrite/create-flows nog steeds eerst `get-theme-file` of `get-theme-files` kunnen vereisen.
- Na `plan-theme-edit` gebruik je bij voorkeur `search-theme-files` met de exacte `keys` uit `nextReadKeys` om eerst compacte snippets te vinden. Voor native-block en andere planner-gestuurde patchflows kan `draft-theme-artifact` daarna de exacte planner-reads server-side auto-hydrateren, zodat de client geen volledige file dumps hoeft mee te slepen.
...
- Gebruik `get-theme-file` om na search het exacte doelbestand op te halen; zonder expliciete `includeContent` blijft die read metadata-first, behalve bij exacte planner-required single-file reads
...
- Voor editflows die net uit `plan-theme-edit` komen, gebruikt `draft-theme-artifact` planner-handoff plus eventuele recente reads. Ontbreekt de exacte read nog, dan probeert de tool die planner-bestanden nu eerst zelf veilig exact te hydrateren voordat de write alsnog wordt geblokkeerd.
