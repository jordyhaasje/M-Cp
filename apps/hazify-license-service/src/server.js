...
import { parseCommaSeparatedList, sha256Hex, normalizeBaseUrl } from "@hazify/mcp-common";
import {
  normalizeShopDomain,
...
async function handleBrandAsset(req, res, url) {
  ...
  const assetPath = path.resolve(APP_ROOT, "assets/brands", rel);
  const brandRoot = path.resolve(APP_ROOT, "assets/brands");
...
async function handleAdminStorageExport(req, res) {
  ...
  try {
    ...
    const backupDir = path.resolve(APP_ROOT, "data/backups");
