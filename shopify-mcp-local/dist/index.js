...
const resolveShopifyAccessTokenFromClientCredentials = async (domain, clientId, clientSecret) => {
    ...
    if (!response.ok) {
        ...
        if (response.status === 400 && /app_not_installed/i.test(text)) {
            throw new Error(`Shopify app not installed on ${domain}. Use shopAccessToken in onboarding, or install/authorize the app for this shop.`);
        }
...
const resolveRemoteContext = async (bearerToken) => {
    const tokenHash = crypto.createHash("sha256").update(bearerToken, "utf8").digest("hex");
    ...
    const domain = String(introspection?.shopify?.domain || "").trim().toLowerCase();
    ...
    const credentialFingerprint = crypto
        .createHash("sha256")
        .update(accessTokenFromIntrospection || `${clientId}:${clientSecret}`, "utf8")
        .digest("hex");
    ...
    if (cachedShopifyClient &&
        ...
        remoteContextCache.set(tokenHash, {
            context,
            expiresAtMs: Date.now() + Math.max(HAZIFY_MCP_CONTEXT_TTL_MS, 10_000),
        });
        return context;
    }
