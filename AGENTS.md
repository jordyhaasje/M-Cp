...
- **`manage-product-options`**: Create, update, or delete product options (e.g. Size, Color). Destructive deletes and valuesToDelete require explicit confirmation and reason.
...
- Als `patch-theme-file` `patch_scope_too_large` teruggeeft, lees altijd eerst het actuele bestand opnieuw met `get-theme-file includeContent=true` op hetzelfde theme target. Pas daarna mag je een preserve-on-edit full-file rewrite naar `draft-theme-artifact mode="edit"` sturen.
...
- `files[].value` in `mode="edit"` moet een volledige actuele filebody zijn die alles behoudt behalve de expliciet gevraagde wijziging. Gebruik nooit een samenvatting, compacte reconstructie of geheugen-rewrite; lossy rewrites worden geblokkeerd.
...
- Voor native product-blocks hoef je het planner-geselecteerde templatebestand na `plan-theme-edit` meestal niet opnieuw te lezen; herlees dat template alleen als placement expliciet gevraagd is.
...
- Templates en config zijn TOEGESTAAN. JSON templates/config krijgen automatische JSON-validatie; Liquid templates krijgen Liquid safety-inspectie.
...
- `templates/*.liquid` wordt in edit mode gevalideerd op renderbare template-markup, Liquid safety en verboden Liquid in `{% stylesheet %}` / `{% javascript %}`.
